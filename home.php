...
// 貼文資訊
$selectPostJoinUser = "SELECT Users.username, Posts.postID, Posts.title, Posts.createdAt FROM Posts JOIN Users ON Users.userID = Posts.userID ORDER BY Posts.createdAt DESC";
// 貼文數量
$selectNumOfPosts = "SELECT COUNT(Posts.postID) AS '貼文數量' FROM Posts;";
// 標籤
$selectTagsName = "SELECT DISTINCT Tags.name FROM Tags;";

$posts = sqlsrv_query($conn, $selectPostJoinUser);
...
<body>
  ...
  <div class="w-full flex flex-row p-16">
    ...
    <main class="w-3/5 flex flex-col gap-y-6 justify-items-center">
      ...
      <?php
      while ($row = sqlsrv_fetch_array($posts, SQLSRV_FETCH_ASSOC)) {
